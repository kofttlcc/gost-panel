...
export const SettingsPage = () => {
  ...
  // 验证面板地址格式
  const checkPanelUrl = (url: string) => {
    const urlPattern = /^https?:\/\/(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*|(?:\d{1,3}\.){3}\d{1,3})(?::\d{1,5})?$/;
  ...
  const addPanelAddress = async () => {
    ...
    // 验证地址格式
    if (!checkPanelUrl(newAddress.trim())) {
      return;
    }
    (window as any).setPanelAddresses = setPanelAddressesFunc
  ...
  const handleApiUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 如果没有 http:// 或 https:// 前缀，且不是刚开始输入，自动添加 http://
    if (value && !value.startsWith('http://') && !value.startsWith('https://')) {
      const isDomainOrIp = /^[a-zA-Z0-9.\-:]*$/.test(value);
      if (isDomainOrIp && value.length > 2) {
        value = `http://${value}`;
        toast.success('已自动添加 http:// 前缀');
      }
    }
    setNewAddress(value);
  };

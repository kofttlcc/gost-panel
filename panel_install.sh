...
# 全局下载地址配置
DOCKER_COMPOSEV4_URL="https://github.com/kofttlcc/gost-panel/releases/download/latest/docker-compose-v4.yml"
DOCKER_COMPOSEV6_URL="https://github.com/kofttlcc/gost-panel/releases/download/latest/docker-compose-v6.yml"
GOST_SQL_URL="https://github.com/kofttlcc/gost-panel/releases/download/latest/gost.sql"

# 根据IPv6支持情况选择docker-compose URL（現已棄用 IPv6，直接使用 IPv4 檔案）
...
generate_jwt_secret() {
  if command -v openssl >/dev/null 2>&1; then
    ...
    head -c 80 /dev/urandom | base64 | tr -dc 'A-Za-z0-9!@#$%^&*()_+' | cut -c 1-64
  fi
}
